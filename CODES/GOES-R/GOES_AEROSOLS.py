...
df = pd.read_csv("/raid3/ilya/GOES_R/GOES_AOD/LatLon_Collocation_G16.csv")
#df = pd.read_csv("/raid3/ilya/GOES_R/GOES_AOD/LatLon_Collocation_G18.csv")
GOES_names = df.iloc[:, 0].tolist()

# Specify the directory path where .txt files are located
AERONET_directory_path = '/raid3/ilya/GOES_R/GOES_AOD/dosadka/files'
...
base_directory = "/raid3/ilya/GOES_R/GOES_AOD/GOES_DATA"

# Define the output directory
output_directory = "/raid3/ilya/GOES_R/GOES_AOD/GOES_OUTPUT"
    ...
    for file in matching_files:
        ...
        try:
            ...
            for i, row in df.iterrows():
                ...
                #iy_arr = [iy-2, iy-1, iy, iy+1, iy+2]
                #ix_arr = [ix-2, ix-1, ix, ix+1, ix+2]
...
input_folder = "/raid3/ilya/GOES_R/GOES_AOD/GOES_OUTPUT"
output_folder = "/raid3/ilya/GOES_R/GOES_AOD/GOES_SITE"
...
input_folder = "/raid3/ilya/GOES_R/GOES_AOD/AERONET_GOES"

for i in range(len(location_list)):
    
    path_AERONET = r'/raid3/ilya/GOES_R/GOES_AOD/dosadka/files/'+location_list[i]+'.txt'
    path_GOES = r'/raid3/ilya/GOES_R/GOES_AOD/GOES_SITE/'+location_list[i]+'.txt'
    
    if os.path.exists(path_AERONET) and os.path.exists(path_GOES):
        ...
        if len(df_AERONET) > 0:
            ...
            if 'AOT550' in df.columns:
                ...
                outpath = input_folder+"/"+location_list[i]+'.csv'
                if len(df) > 0:
...
for i in range(len(csv_files)):
    df = pd.DataFrame()
    directory = input_folder+"/"+csv_files[i]
...
output_folder = "/raid3/ilya/GOES_R/GOES_AOD/CORRELATION_PLOTS"
os.makedirs(output_folder, exist_ok=True)

for i in range(len(name_list)):
    df = pd.DataFrame()
    directory = input_folder+"/"+name_list[i]+'.csv'
    df = pd.read_csv(directory)
    ...
    if len(df) > 0:
        ...
        plt.ylabel('GOES-R AOT550')
        plt.title('AERONET Site: '+str(name_list[i])+'          N = '+str(len(df))+'\n'+str(trendline)+'          R-coefficient: '+str(round(corr,4)))
        plt.savefig(str(output_folder)+'/'+str(name_list[i])+'.png', dpi=300)
        plt.clf()
